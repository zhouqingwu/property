...
function wpestate_estate_property_design_agent_details_intext_details($attributes,$content = null){
    ...
    foreach($matches[0] as $key=>$value){
        ...
        if($element =='name'){
            ...
            $preview            = wp_get_attachment_image_src(get_post_thumbnail_id($agent_id), 'agent_picture_single_page');
    ...
    foreach ($post_attachments as $attachment) {
            ...
            $return_string.= '<li class="" style="margin:0px '.($margin/2).'px '.$margin.'px '.($margin/2).'px;">
                            <a href="'.$full_prty[0].'" rel="prettyPhoto[pp_gal]" class="prettygalery" > 
                                <img  src="'.$preview[0].'"  alt="'.$attachment->post_excerpt.'" style="max-width:'.$maxwidth.'px;" />
                            </a>
                            </li>';
...
function wpestate_estate_property_design_agent_contact($attributes,$content = null){
    ...
    if ( $contact_form_7_agent==''){ 
        ...
        <input type="submit" class="wpb_button  wpb_btn-info wpb_btn-large"  id="agent_submit" value="'. __('Send Message', 'wpestate').'">
...
function wpestate_shortcode_listing_slider($propid,$showmap){
    ...
    if ( $header_type == 0 ){ // global
        if ($global_header_type != 4){
                ...
                if($showmap=='yes'){
                    $return_string.='<div id="slider_enable_slider" data-placement="bottom" data-original-title="'.__('Image Gallery','wpestate').'" class="slideron '.$no_street.'"> 3<i class="fa fa-picture-o"></i>         </div>';
...
function wpestate_property_page_design_acc($attributes,$content = null){
    ...
    $return_string.= estate_property_page_generated_acc($css_class,$propid,$style,$description,$property_address,$property_details,$amenities_features,$map,$walkscore,$floor_plans,$page_views);
    $return_string.='</div>';
...
function estate_property_page_generated_acc($css_class,$propid,$style,$description,$property_address,$property_details,$amenities_features,$map,$walkscore,$floor_plans,$page_views){
...
function wpestate_property_page_design_tab($attributes,$content = null){
    ...
    $return_string.= estate_property_page_generated_tab($propid,$description,$property_address,$property_details,$amenities_features,$map,$walkscore,$floor_plans,$page_views);
...
function estate_property_page_generated_tab($propid,$description,$property_address,$property_details,$amenities_features,$map,$walkscore,$floor_plans,$page_views){
    $walkscore_api      =   esc_html ( get_option('wp_estate_walkscore_api','') );
    $show_graph_prop_page   = esc_html( get_option('wp_estate_show_graph_prop_page', '') );
    $random             =   rand(0,99999);
